...
| 原文语料（本机） | `.canon/**/*.txt` | epub 切出的整卷原文，只作**构建期语料**用：切 offtext、校文风指标、给 `src/data/eventnotes.ts` 逐字摘录做定位。版权归原作者，**不宜随仓库分发** —— 已在 `.gitignore` 里排除；早先已入库的那批量要彻底撤下得改写历史，属于仓库所有者的决定。 |
