...
- **这一回合以言万心叶的意志为中心。** 他明确说了要做什么、要达成什么，那句话就是**既成前提**，不是待议的提议 —— 照他说出来的结果往下写：不打折、不替他换一种做法、也不要让在场者拿「做不到／来不及／另有隐情」把它原地挡回去。**他写下的剧情，就是已经发生的事**：此后所有叙述与在场者的反应，都建立在它之上。他推到哪儿，故事就跟到哪儿；只有他**没说死**的地方，才交给原著走向与在场角色的自主反应去填。
...
- 大纲的用途是**分寸与事实**：这个人此刻会怎么说话、这件事绕不开的关节是什么、他还不该知道什么。它不是一张「这几件事非发生不可」的清单 —— 硬把情节推回原位，比偏离原著更糟。
...
「${body}」
这一回合就照它写。上面的【事件大纲 · 原文走向】、实施细则里的情节线与落点、【收束衔接 · 后接事件】，全部让位：
· 结果照他写的算。他做成了就是做成了 —— 不要写成「他试了，但没成」，也不要让在场者把它驳回、替他换一种做法，或者拿「做不到／来不及／另有隐情」原地挡回去。
· 他写下的行动**已经发生**：在场者只能对它作出反应，不能当作没发生，也不能撤销它。
· 他推到哪儿，就以哪儿为这一段的了结处收束 —— 那正是把 diverged 置 true 的时候。不要为了让收束对上原著的落点，把人往原作那条线上拽。
...
function briefSection(ev: TimelineEvent): string {
  ...
  if (b.done?.length) {
    seg.push('四、原文里这一段的落点（原著是在这几条上收的；言万心叶若把它推到别处，'
      + '就按**实际结果**收束并给 diverged，不要为了凑上这几条而硬拉）\n'
      + b.done.map((x) => ` · ${x}`).join('\n'))
  }
  if (b.taboo?.length) {
...
更新规则：本回合言万心叶的行动若改变了某登记键所指的状态，就把该键连同新值写进下方事件指令的 flag（未变化的键不要写）；先前登记过的键要更新其值、不要另建同名键；若行动带来值得长期记录的新状态（约定／承诺／隐瞒／共同秘密／称号／处境档位等）可新建键，键名用英文小写加下划线、无空格、≤48 字符；不要为了写而写。`
  ...
  const will = ctx.operatorAction ? willSection(ctx.operatorName || '言万心叶', ctx.operatorAction) : ''
...
言万心叶自己写下的剧情**压过本节的一切**：他说这一段怎么走，就怎么走。）
