 ...
 * 而操作员不在那二十五人的档案名录里（见 `castmeta` 的操作员条目）。把他并进去，
 ...
 *
 * 约会专线（components/DateLane.tsx）走的是**同一个口子** —— 那一路也有自己的
 * 预设段（`preset.pre` / `preset.post`），同样会把行首写法盖掉；它少了这一段，
 * 表现就是「台词不切气泡，整段落进旁白」。`castRef` 只改「与上方哪一节对名字」，
 * 正文一字不差：那一路的在场名单在 rendezvousPrompt 的「在场角色」那一节。
 */
export function speechContract(castRef = '【本事件出场角色】'): string {
...
· 名字用其**全名**（与上方${castRef}里的写法一致），冒号用中文全角「：」；
· 冒号后可直接接台词，也可用「」把台词括起；
