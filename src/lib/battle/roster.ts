  ...
  nyau: {
    ...
    skills: [
      ...
      /* 「掷入真空」原按驱逐写（1.7，带沉默）。驱逐是**取材**那一档的类 ——
         ...
         它找不到该往哪儿充，破绽也一起露在外面。
         ⚠️ 沉默跟着丢了（牵制的布尔白名单里没有它，见 atlas 的 `allow`）——
         这是归位的代价，不是漏写；那一句台词照旧挂在名字上。
         ——
         **2026-09-17 主人点名升成「对格尔的特攻」（脏器公寓那一场）：**
         三样一起给 —— 破绽尽碎（防归零、**一个回合**不重凝）+ 减攻 0.5 + 易伤 0.6，
         后两样都是**顶格**（`EFF_BAND` 的 frail / mark 上限就是这两个数，见 tuning）。
         口径是「整个升级、不加判定」：这手对**谁都**这么重，不挑对手，
         所以引擎里没有、也不要加「只对格尔生效」那类暗桩（见 endfoes 的格尔一条）。
         ——
         **四样东西共用一个钟**（`rounds: 1`，2026-09-17 补的那一格）：
         · `guardClear` 走**回合**那条钟（`TUNING.guardClearRounds` = 1，收拍一次）——
           主人要的那条。他说的「一个回合（6 拍）」里的 6 是按满编六人算的手数，
           **不是**让你往这一格上写 6（那会是六个回合）。
         · `mark` / `frail` / `slow` 是**负面**，天生吃不到按回合那道闸
           （`wearsByRound` 只给增益），本来只数「目标自己出手几次」——
           而格尔一回合里出手几次，要看速度差与断拍，玩家算不出来。
           所以这一手**自己认领**回合钟：`rounds: 1` 让它们挂上 `rt`，
           与 `guardMend` 同一步到点 —— 护盾凝回去的那一刻，这几条一起散。
         ⚠️ `turns: 2` 留着没动，它是**兜底**（目标一直不出手时别挂到天荒地老），
           别把它当成「两回合」——那一栏数的是**拍**（见 SkillSpec.turns 的头注）。 */
      place('牵制', {
        id: 'nyau-void', name: '掷入真空', axis: '反现实亲和', fx: 'seal',
        turns: 2, rounds: 1,
        effect: { guardClear: true, mark: 0.6, frail: 0.5, slow: 0.45 },
        desc: '把远处的敌人直接与子弹换位，掷进无声的宇宙 —— 那里面没有方向：'
          + '它充不起来，破绽也整个掀开、再也凝不回去。',
  ...
  'majina-abram': {
    ...
    skills: [
      ...
      place('牵制', {
        ...
        /* noHeal（2026-09-18 主人点名加的）：同一根钉子往前推一步 ——
           「禁止」禁到底就是**连愈合也一并禁掉**（时长见 TUNING.noHealRounds，
           整整一个回合，和 guardClear 同一条收拍钟）。
           它压的是「回血」那两条路（被动自愈 + 治疗），不碰复活 ——
           见 SkillEffect.noHeal 头注。落在这一手上是因为它本来就是「出口即禁止」
           的那一句：短句钉关节，长句钉得更深 —— 而这一句钉的是**本来会长回去的地方**。 */
        effect: { mark: 0.42, slow: 0.32, stall: 1, noHeal: true },
        desc: '她说出那句话的同时，钉子就出现在对手的关节上 —— 那一处动不了，'
          + '这一拍的手也就抬不起来。钉子谁都碰不到，挣得越急，反而掉得越快。'
          + '而被钉住的那一处，这一整个回合都长不回去了。',
        line: '「禁止。你就这样钉着吧。」',
      }),
      place('驱逐', {
