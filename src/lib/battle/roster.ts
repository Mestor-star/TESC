    ...
    skills: [
      ...
      /* 「掷入真空」原按驱逐写（1.7，带沉默）。驱逐是**取材**那一档的类 ——
         ...
         **2026-09-17 主人点名升成「对格尔的特攻」（脏器公寓那一场）：**
         ...
         ——
         ⚠️ 两样东西**不共用一个钟**，别以为它们同时到点：
         · `guardClear` 走**回合**那条钟（`TUNING.guardClearRounds` = 1，收拍一次）——
           主人要的那条。他说的「一个回合（6 拍）」里的 6 是按满编六人算的手数，
           **不是**让你往这一格上写 6（那会是六个回合）。
         · `mark` / `frail` 是**负面**，引擎里吃不到按回合那道闸（`wearsByRound` 只给增益），
           它们数的是**目标自己出手几次**（`turns`，兜底 `buffTurnsCap`）。
           所以「一回合」写不进 `turns` —— 那栏最小是 1（= 格尔整整一次出手），
           反而比一回合还长；要它们也精确钉在回合上，得另立一个键，届时再说。 */
      place('牵制', {
        id: 'nyau-void', name: '掷入真空', axis: '反现实亲和', fx: 'seal',
        turns: 2, effect: { guardClear: true, mark: 0.6, frail: 0.5, slow: 0.45 },
