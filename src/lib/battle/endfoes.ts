...
const SPECS: EndSpec[] = [
  ...
  {
    codex: 'fanatic-ger',
    /* 图鉴 Stage6『动摇』，但**一个人就把苍之学园与卡乌斯学院的联合小队打残过**
       —— 所以按 7 站，比那座馆高一档。
       ---
       **2026-09-17（主人）：脏器公寓那一场只打他一个。**
       原先 v1-5 是三形态接力（公寓本体 → 格尔 → 黑曜石），现在直接从格尔开打、
       倒在他这儿就收场 —— 所以这一条**不再写 `next`**。公寓本体（organ-apt）那一份
       仍在表里、也仍留着指向他的 `next`（那是原文里布下这一场的次序，作为图鉴资料是真的），
       只是 v1-5 不再从它起手；黑曜石那一份同样留在表里当图鉴，这一场不揭开它。
       链是**写出来的、不是引擎默认给的** —— 去掉这一行就是「到此为止」，
       这一条同时也是 §⑧ 那条断言的活样本。 */
    ratedAs: 7,
    atk: {
    ...
    /* 破绽挂在「意志力」上 —— 而「掷入真空」那手特攻是**掀开**它，不是削它
       （见 roster 的 nyau-void / engine 的 clearGuard）：对上轴才削得动的那一层，
       在这一手面前整个归零，且这一回合里不许重凝。 */
    guardAxis: '意志力',
    /* 不写 `next`：他倒下就是这一场收场（2026-09-17，见上）。 */
  },
  {
    codex: 'obsidian',
