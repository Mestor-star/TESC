...
export const ARCH: Record<string, Arch> = {
  ...
  牵制: {
    ...
    /* 放行 guardClear（破绽尽碎，2026-09-17）：这一类本来就是「**压在一个人身上**」——
       ...
       放行 noHeal（禁止命令，2026-09-18）走的是**同一条理**：
       「压在一个人身上」压到底，就是**连它的喘息也一并按住** ——
       压住它 → 它动不了 → 它也没得挡 → 它更回不上来。四句是一句。
       别的框架**一样不放**：禁疗是会改写整场节奏的压制（对面有回血的首领，带不带这一手
       是两个难度），要留给「专程为它上一个辅助手」这个决定，不能顺手挂在别类的效果里。 */
    allow: ['guardClear', 'noHeal'],
    bar: true,   // 这一整类都以动条为打法
  },
  重压: {
