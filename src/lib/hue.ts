/* ============================================================
   ...
   2026-09-15 整体改版把底换成白底之后，同一批颜色**当文字**用就糊了 ——
   最浅的 `#cfcfef` 在白底上只有 1.52:1，写出来几乎看不见。
   （2026-09-16「冷白全息」那一版把底从暖白挪到冷白、墨从暖墨换成冷墨，
   这一支的算法与门槛一个字没动 —— 底仍是白的，尺子就仍是那把。）
 ...
 *  2026-09-16 随「冷白全息」那一版换过：上一版是暖墨 `#241f3a`，现在是冷墨 `#0e1729`。 */
const INK = { r: 0x0e, g: 0x17, b: 0x29 }
 ...
 * 实测不带余量时，退出来的值会落在 4.4996:1 这种地方（看着是 4.50，其实没过），
