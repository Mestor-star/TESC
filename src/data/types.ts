...
export type CgRef = string | {
  ...
  cast?: string[]
  /**
   * **第三道窄法**：这一张要**这一场先走到那一步**才进候选（节拍 id，见
   * `lib/rendezvous.ts` 的 `DATE_BEATS`）。缺省 = 没有前提。
   *
   * 与 `cast` 是**叠着**的两道，不是互相替代：「买完衣服试穿那张」两样都带 ——
   * 露娜不在场进不来，还没走到试穿那一步也进不来。
   *
   * 节拍是**闭集**（只有 `DATE_BEATS` 里登记过的能落），因为自由串守不住「先走到
   * 这一步才给」这条硬规矩 —— 模型换个说法写就绕过去了。写错一个字 = 这张画
   * **永远差一步**（不报错、不告警），所以 mech 有一条断言对着它。
   *
   * 记在**这一场**身上（`Rendezvous.beats`），不是 `world.flags` —— 后者跨线持久，
   * 那就成了「买过就永久算」，与「只算这一场」正相反。
   */
  needs?: string
  /**
   * 图所在的**子目录**（相对 `public/cg/`，如 `lunaNSFW/正常位`）。缺省 = 顶层。
   *
   * `public/cg` 是平铺取图的（`lib/cg.ts` 拼 `<cgId>.<ext>`）—— 想把素材按戏码
   * 分文件夹收，就得在这里把路指过去。**换图仍然不用改码**：目录不变的前提下，
   * 丢个同名的 `.webp` 进去照样点亮（候选链 webp → png → jpg 照旧）。
   */
  dir?: string
  /**
   * 这一张画有**几个变体**（文件是 `<id>-1`、`<id>-2`… 画面只有细微差别）。缺省 1 = 单张，文件名不带号。
   *
   * **不是动画帧** —— 就是同一场戏多画了几版，每次被触发就换下一个、转着圈来
   * （转了几次落在存档的 `WorldState.cgVariant`，见 `Terminal` 的 `setCg`）。
   */
  variants?: number
}
...
export interface WorldState {
  ...
  /**
   * 各约会**这个槽位被触发过几次**：约会 id（`d:uuid`）→ 次数。
   *
   * 只为一件事存在：同一个槽位画了好几版时（`CgRef.variants`），每次被触发就换
   * 下一版，转着圈来。**记的是「触发了几次」而不是「在用第几张」** —— 用第几张
   * 由 `lib/cg.ts` 的 `cgVariantId` 拿这个数取模现算，往后改变体张数不必迁存档。
   * 计数挂在约会上、按约会各算各的：同样的次数既服务当前槽位，换了槽位也接着走。
   * 旧档缺这一栏 → 空表，下一次触发当第 1 张。
   */
  cgVariant?: Record<string, number>
  /**
